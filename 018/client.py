...
class DualDecomp:
    ...
    #    self.sources = [u"ich kaufe sie eine katze", u"je vous achète un chat",]
        ...
        with open(l0_sentfile) as f0, open(l1_sentfile) as f1:
            self.sources = zip(f0.read().split('\n'), f1.read().split('\n'))
...
#                return proxy.translate(params)
                ...
                print("Unsuccessful connection after {} tries.".format(tries))
                time.sleep(15)
                continue
        return result
    ...
    def writePT(self, lang_index, result, prev_pt, src_pair, write=False):
...
#        print("\n{} written...".format(pt_name))
    ...
    def writeLM(self, lang_index, Lambdas, max_order, write=False):
...
#        lm_file.write("\n\\end\\\n")
        lm += "\n\\end\\\n"
#        lm_file.close()
        self.reloading_LMs[lang_index] = lm

        if write:
            with open(lm_name) as lm_file:
#            global reloading_LM
#                self.reloading_LMs[lang_index] = lm_file.read()
                lm_file.write(reloading_LMs[lang_index])

#        print("\n{} written...".format(lm_name))

    #######################################################i

    def dualDecomposition(self, max_iters=50, eta=0.3, max_order=3):

        print("Starting dual decomposition...")

        with open("translations.0.txt","w") as out0, open("translations.1.txt","w") as out1:

            src0translations = []
            src1translations = []

            lm_text = ""
            print("reading static LM")
            with open("static.lm") as lmfile:
                lm_text = lmfile.read()

            for lang_index, src_lang in enumerate(self.langs):
                pt_name = "static.{}.pt".format(lang_index)
                with open(pt_name) as ptfile:
                    print("reading static PT # {}".format(lang_index))
                    self.reloading_PTs[lang_index] = ptfile.read()

                print("saving static LM # {}".format(lang_index))
                self.reloading_LMs[lang_index] = lm_text

            print("finsihed loading static PTs and LMs")

            for src_pair in self.sources:

                print("translating pair {} :: {}".format(src_pair[0], src_pair[1]))

                if src_pair[0].startswith("<"):
                    assert src_pair[0].endswith(">")
                    assert src_pair[1].startswith("<")
                    assert src_pair[1].endswith(">")
#                    src0translations.append(src_pair[0])
#                    src1translations.append(src_pair[1])
                    out0.write(src_pair[0]+'\n')
                    out1.write(src_pair[1]+'\n')
                    continue
                ...
                for lang_index, src_lang in enumerate(self.langs):
                    ...
                    print("language={} translation={}".format(src_lang, translation))
    ...
    #                        pt_name,
                ...
                for i in range(max_iters):
                    print("iteration {}:\n\ttrans0= {}\n\ttrans1= {}".format(i, translations[0], translations[1]))
    ...
    #                                    + lm_name + recordSeparator + self.reloading_LMs[lang_index]
                        ...
                        contextScope = "TranslationModel0" + recordSeparator + self.reloading_PTs[lang_index] \
                                        + groupSeparator \
                                        + "LM0" + recordSeparator + self.reloading_LMs[lang_index]

                        print("consulting dynamic moses...")
    ...
    #                            pt_name,
            ...
            #                    handleOOV(word, lang_index)
                ...
                out0.write(re.sub("<s>(.+)</s>", "\g<1>", translations[0]).strip()+'\n')
                out1.write(re.sub("<s>(.+)</s>", "\g<1>", translations[1]).strip()+'\n')
    ...
    #            out1.write("\n".join(src1translations)+"\n")
        ...
        for lang_index, src_lang in enumerate(self.langs):
            pt_name = "reloading.{}.pt".format(lang_index)
            lm_name = "reloading.{}.lm".format(lang_index)
            with open(pt_name,"w") as ptfile:
                ptfile.write(self.reloading_PTs[lang_index])
            with open(lm_name,"w") as lmfile:
                lmfile.write(self.reloading_LMs[lang_index])
        
        print("\nAll source pairs translated.  Exiting.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--max_iters', dest='iters', default=50, action='store', help='max # of iters for outputs to converge')
    parser.add_argument('-e', '--eta', dest='rate', default=0.3, action='store', help='learning rate')
