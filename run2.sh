...
../../bin/mosesserver -v 3 -f moses.ini --server-port 8091 &> log &
dynamic_server_pid=${!}
...
kill ${static_server_pid} || echo "Failed to kill static Moses server ${static_server_pid}"
kill ${dynamic_server_pid} || echo "Failed to kill dynamic Moses server ${dynamic_server_pid}"
